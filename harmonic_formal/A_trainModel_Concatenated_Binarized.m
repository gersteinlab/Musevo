function A_trainModel_Concatenated_Binarized

% Train evolutionary model using both harmonic and formal features
...
%   Columns 1 through 9
% 
%     2.3013    2.3744    2.3761    2.3761    2.3761    2.3761
...
featThres = 40; % feat must occur in # songs (optimal 40)

% load / prune / prepare dataset
load('models/k1_dedup','X_tr','X_te','nFeat');
nFeat_1 = nFeat;
ls = [];
for i = 1:length(ids)
    ls = [ls str2num(ids{i})];
...
save('datMats','X_tr_all','X_te_all');
...
for it = 1:10
    ...
    % optimize thetas
    fn = @(theta) -optFn(theta,X_tr_all,Q);
    if it==1
        theta_in = theta0;
    else
        theta_in = theta1;
    ...
    [val S_idx_tr U] = optFn(theta1,X_tr_all,Q);
    ...
    if corrs(end)==max(corrs)
        save(fout_best,'theta0','thetas','Qs','corrs','Ls','X_tr_all', ...
            'X_te_all','X_tr_struct','X_te_struct','feat_key_struct');
    end
...
save(fout,'theta0','Q00','thetas','Qs','corrs','Ls','X_tr_struct', ...
    'X_tr_all','X_te_all','X_te_struct','feat_key_struct');
